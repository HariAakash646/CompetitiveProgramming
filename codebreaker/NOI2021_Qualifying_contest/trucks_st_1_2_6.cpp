...
int main()
{
    ...
    if (n <= 5000 && q <= 5000)
    {
        frange(_, q)
        {
            ...
            if (v)
            {
                ...
                scd(x);
                scd(y);
                lli d = dfs16(x, y, 0, 0).f;
                printf("%lld\n", (dfs26(x, y, 0, d).f + (g * dfs36(x, y, 0, 0).f) % mod) % mod); // dfs2(x, y, 0, d).f + g *
            }
            else
            {
                int x, y;
    ...
    else
    {
        ...
        frange(_, q)
        {
            ...
            printf("%lld\n", (toll[max(0LL, min(lli(n), dist[x] + dist[y] - 2 * dist[lca]))] % mod + ((((dist[x] + dist[y] - 2 * dist[lca] + 5 * mod) % mod) * g) % mod)) % mod);
        }
    }
}