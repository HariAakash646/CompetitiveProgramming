...
struct LazySegTree
{
    ...
    void update(int x, int l, int r, int lx, int rx, T val)
    {
        ...
        if (lx <= l && r <= rx)
        {
            segtree[x].s = operation(segtree[x].s, val);
            return;
        }
        int mid = (l + r) / 2;
        if (lx <= mid)
...
int main()
{
    ...
    frange(i, q)
    {
        ...
        cin >> x;
        tree.update(id[x - 1]);
        int v = min(c, tree.query(0, n - 1));
        int k = tree.query(0, v - 1);
        cout << v - k << "\n";
    }
}