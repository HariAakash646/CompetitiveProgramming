...
int main() {
    ...
    forr(i, 1, n) v2[i] = mp((vec[i] - vec[i-1] + k) % k, -i);
    v2[n] = mp((k-vec[n-1])%k, -n);
    ...
    for(auto p : v2) {
        int id = -p.s;
        segtree.update(id, n, -k);
        // printf("%lld ", segtree.query(0, n));
        // frange(i, n+1) {
        //     printf("%lld ", segtree.query(i, i));
        // }
        // printf("\n");
        lli x = k - p.f;
